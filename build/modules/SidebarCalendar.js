...
export default class SidebarCalendar {
    ...
    // displayers
    displayHeading() {
        const sidebarCalendarHeading = document.querySelector('#sidebar-calendar-date');
        const { year, month } = this.date.parseDateFromMonthOffset(this.monthOffset);
    ...
    createCell(dayNumber, notCurrMonth = false) {
    ...
    bindSidebarCalendarControls() {
        const nextPeriodBtn = document.querySelector('#sidebar-calendar-next-period');
        const prevPeriodBtn = document.querySelector('#sidebar-calendar-previous-period');
